<?php
/**
 * REST API Multi-Currency admin controller.
 *
 * Read-only inspection endpoint that surfaces the currency manager's runtime
 * state to the React settings panel. Lives in the wc/v3 namespace because it's
 * an admin-capability resource, not a /me/* customer view.
 *
 * Response shape:
 *   {
 *     base_currency:        "USD",
 ...
 *     mode_setting:         "single_base" | "per_currency",   // raw setting value
 *     per_currency_enabled: bool,                              // is the feature flag on
...
if ( ! class_exists( 'TeraWallet_REST_Multicurrency_Controller' ) ) {

	/**
	 * Multi-currency admin controller.
	 */
	class TeraWallet_REST_Multicurrency_Controller extends TeraWallet_REST_Controller_Base {
		...
		/**
		 * Manage-wallet-settings capability check (mirrors the settings controller).
		 *
		...
		public function get_state() {
			...
			// Effective mode — what the wallet actually does at runtime, accounting
			// for the feature flag that gates per_currency until PR4 GA.
