	...
	class WCMp_Gateway_Wallet extends WCMp_Payment_Gateway {
			...
			if ( $this->validate_request() ) {
				if ( $this->process_wallet_payment() ) {
					...
					if ( $this->transaction_id ) {
						return array(
							'message'        => __( 'New transaction has been initiated', 'woo-wallet' ),
							'type'           => 'success',
		...
		public function validate_request() {
			...
			if ( $this->enabled != 'Enable' ) {
				$this->message[] = array(
					'message' => __( 'Invalid payment method', 'woo-wallet' ),
					'type'    => 'error',
				);
				return false;
			}
			if ( $this->transaction_mode != 'admin' ) {
				...
				if ( $this->get_transaction_total() > $thesold_amount ) {
					...
					$this->message[] = array(
						/* translators: %s: minimum threshold amount */
						'message' => sprintf( __( 'Minimum threshold amount to withdrawal commission is %s', 'woo-wallet' ), $thesold_amount ),
						'type'    => 'error',
					);
					return false;
				}
		...
		private function process_wallet_payment() {
			...
			$transaction_id  = woo_wallet()->wallet->credit( $this->vendor->id, $amount_to_pay, __( 'Commission received for commission id ', 'woo-wallet' ) . $for_commissions, array( 'for' => 'vendor_commission' ) );
			if ( $transaction_id ) {
				return true;
			}
			return false;
