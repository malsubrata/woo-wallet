...
if ( ! class_exists( 'Woo_Wallet_Ajax' ) ) {
	...
	class Woo_Wallet_Ajax {
		 ...
		 *
		 * @throws exception WooCommerce exceptions.
		 * @throws Exception WooCommerce exceptions.
		 */
		...
		public function woo_wallet_user_search() {
			...
			if ( apply_filters( 'woo_wallet_user_search_exact_match', true ) ) {
				$user = get_user_by( apply_filters( 'woo_wallet_user_search_by', 'email' ), $_REQUEST['term'] ); // phpcs:ignore
				...
				$users = get_users(
					array(
						'blog_id'        => $id,
						'search'         => '*' . $_REQUEST['term'] . '*', // phpcs:ignore
		...
		public function draw_wallet_transaction_details_table() {
			...
			$start  = isset( $_POST['start'] ) ? $_POST['start'] : 0; // phpcs:ignore
			$length = isset( $_POST['length'] ) ? $_POST['length'] : 10; // phpcs:ignore
			$search = $_POST['search']; // phpcs:ignore
			$args   = array(
				'limit' => "$start, $length",
			);
