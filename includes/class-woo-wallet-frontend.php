...
if (!class_exists('Woo_Wallet_Frontend')) {
    ...
    class Woo_Wallet_Frontend {
        ...
        public function woocommerce_checkout_order_processed($order_id, $posted_data, $order) {
            ...
            if (!apply_filters('woo_wallet_disable_partial_payment', (!is_full_payment_through_wallet() && is_user_logged_in() && ((isset($_POST['partial_pay_through_wallet']) && !empty($_POST['partial_pay_through_wallet'])) || 'on' === woo_wallet()->settings_api->get_option('is_auto_deduct_for_partial_payment', '_wallet_settings_general')) && !is_wallet_rechargeable_order(wc_get_order($order_id))))) {
                ...
                $order->set_total($order->get_total('edit') - $current_wallet_balance);
