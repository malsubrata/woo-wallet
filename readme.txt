...
= v1.6.13 (Unreleased) =
– **Fix:-** Cashback stopped being credited on stores that had saved the settings page at least once. The "Process cashback" field listed WooCommerce's order statuses using their internal prefixed names (`wc-processing`), while the order status the plugin actually waits for is written without that prefix — so the moment a store saved its selection, the plugin began listening for an order status that never occurs, and cashback silently stopped. Stores that had never opened the settings page were unaffected, which is why this survived so long. Both places that read the setting now go through one shared reader that accepts either form, so affected stores start crediting again on update with no need to re-save. The "Recalculate cashback" order action, which had been silently doing nothing for the same reason, works again too. The `wallet_cashback_order_status` filter is unchanged and existing workarounds hooked to it keep working.
– **Fix:-** The cashback type and cashback rule settings could be saved as an empty value. Neither field declared a default, so the settings screen submitted an empty value for a field the admin never touched, while the screen itself displayed the first option as though it were selected — the stored setting and the visible label disagreed. The save routine now holds these fields to the options they actually offer, and both fields declare their default. Stores already holding an empty value are unaffected: it was, and still is, read as "Percentage".
