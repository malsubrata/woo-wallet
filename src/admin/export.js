...
( function ( $, window ) {
	 ...
	 * @param $form
	 */
	const teraWalletExportForm = function ( $form ) {
		this.$form = $form;
		this.xhr = false;
		// Initial state.
		...
		this.$form
			.find( '.terawallet-exporter-type' )
			.on( 'change', { teraWalletExportForm: this }, this.onChangeType )
			.change();
	};
	...
	teraWalletExportForm.prototype.onSubmit = function ( event ) {
		...
		event.data.teraWalletExportForm.$form
			.find( '.terawallet-exporter-progress' )
			.val( 0 );
		event.data.teraWalletExportForm.$form
	 ...
	 * @param filename
	 */
	teraWalletExportForm.prototype.processStep = function (
		step,
		data,
		columns,
		filename
		...
		const $this = this,
			export_type = $( '.terawallet-exporter-type' ).is( ':checked' )
				? 'balance'
				: 'transactions',
