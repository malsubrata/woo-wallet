...
function SaveButton( { onClick, saving, saved } ) {
	return (
		...
		>
			...
			{ saving ? __( 'Saving…', 'woo-wallet' ) : saved ? __( 'Saved!', 'woo-wallet' ) : __( 'Save Changes', 'woo-wallet' ) }
		</button>
	);
}
...
function evaluateCondition( cond, values ) {
	...
	if ( Array.isArray( equals ) ) return equals.includes( val );
	return String( val ) === String( equals );
}

function isVisible( field, sectionValues ) {
	if ( ! field.show_if ) return true;
	const conds = Array.isArray( field.show_if ) ? field.show_if : [ field.show_if ];
...
function StackedField( { field, value, onChange } ) {
	...
	if ( field.type === 'checkbox' ) {
		return <FieldComponent value={ value } onChange={ onChange } field={ field } />;
	}
	...
	return (
		<div style={ { display: 'flex', flexDirection: 'column', gap: 10 } }>
			<div>
				<label
					style={ { fontSize: 13, fontWeight: 600, color: 'var(--ww-text-label)', display: 'block' } }
					dangerouslySetInnerHTML={ { __html: field.label } }
				/>
				{ field.hint && (
					<p
						style={ { margin: '3px 0 0', fontSize: 12, color: 'var(--ww-text-muted)', lineHeight: 1.5 } }
						dangerouslySetInnerHTML={ { __html: field.hint } }
					/>
				) }
			</div>
			<FieldComponent value={ value } onChange={ onChange } field={ field } />
		</div>
	);
}
...
function renderFieldRows( fields, sectionValues, onChange ) {
	...
	fields.forEach( ( field ) => {
		if ( ! isVisible( field, sectionValues ) ) return;
		if ( field.half ) {
			pushHalf( field );
		} else {
			flushHalf();
...
function partitionFields( fields ) {
	...
	fields.forEach( ( field ) => {
		...
		if ( field.group_title && ! group.title ) group.title = field.group_title;
		if ( field.group_description && ! group.description ) group.description = field.group_description;
		if ( field.group_type && group.type === 'regular' ) group.type = field.group_type;
		if ( field.group_logo && ! group.logo ) group.logo = field.group_logo;

		if ( ! group.master && field.type === 'checkbox' && field.group_title ) {
			group.master = field;
		} else {
	...
	const regularGroups = allGroups.filter( ( g ) => g.type !== 'gateway' );

	return { groups: regularGroups, gatewayGroups, loose };
}

function GroupCard( { group, values, onChange } ) {
	const master = group.master;
	const masterValue = master ? ( values[ master.name ] ?? master.default ?? '' ) : 'on';
	const masterChecked = masterValue === 'on' || masterValue === true || masterValue === 1 || masterValue === '1' || masterValue === 'yes';
	const hasVisibleChildren = masterChecked && group.children.some( ( f ) => isVisible( f, values ) );

	return (
		<div style={ {
			background: 'var(--ww-surface)',
			border: `1.5px solid ${ masterChecked ? 'oklch(0.55 0.16 185 / 0.35)' : 'var(--ww-border)' }`,
			borderRadius: 14,
			...
			{ ( group.title || group.description ) && (
				<div style={ {
					...
					<h3
						style={ { fontSize: 15, fontWeight: 600, color: 'var(--ww-text-heading)', margin: 0 } }
						dangerouslySetInnerHTML={ { __html: group.title } }
					/>
					{ group.description && (
						<p
							style={ { fontSize: 12, color: 'var(--ww-text-muted)', margin: '4px 0 0' } }
			...
			<div style={ {
				...
				background: hasVisibleChildren ? 'var(--ww-section-body-bg)' : 'transparent',
				...
				{ hasVisibleChildren && renderFieldRows( group.children, values, onChange ) }
			</div>
		</div>
	);
}
	...
	paypal: () => (
		<svg viewBox="0 0 48 48" width="22" height="22">
			...
			<text x="50%" y="68%" textAnchor="middle" fill="white"
				fontSize="20" fontWeight="700" fontFamily="sans-serif">P</text>
		</svg>
	),
	stripe: () => (
		<svg viewBox="0 0 48 48" width="22" height="22">
			...
			<text x="50%" y="68%" textAnchor="middle" fill="white"
				fontSize="20" fontWeight="700" fontFamily="sans-serif">S</text>
		</svg>
	),
	razorpay: () => (
		<svg viewBox="0 0 48 48" width="22" height="22">
			...
			<text x="50%" y="68%" textAnchor="middle" fill="white"
				fontSize="20" fontWeight="700" fontFamily="sans-serif">R</text>
		</svg>
	),
	bacs: () => (
		<svg viewBox="0 0 48 48" width="22" height="22">
			...
			<text x="50%" y="68%" textAnchor="middle" fill="white"
				fontSize="20" fontWeight="700" fontFamily="sans-serif">B</text>
		</svg>
	),
	cashfree: () => (
		<svg viewBox="0 0 48 48" width="22" height="22">
			...
			<text x="50%" y="68%" textAnchor="middle" fill="white"
				fontSize="20" fontWeight="700" fontFamily="sans-serif">C</text>
		</svg>
	),
	paystack: () => (
		<svg viewBox="0 0 48 48" width="22" height="22">
...
function GatewayCard( { group, values, onChange } ) {
	...
	const master = group.master;
	const masterValue = master ? ( values[ master.name ] ?? master.default ?? '' ) : 'on';
	const masterChecked = masterValue === 'on' || masterValue === true || masterValue === 1 || masterValue === '1';
	...
	const modeField = group.children.find( ( f ) => /_test_mode$/.test( f.name ) );
	const isTestMode = modeField ? ( values[ modeField.name ] === 'on' ) : false;

	const statusLabel = masterChecked
		? ( isTestMode ? __( '● Sandbox mode', 'woo-wallet' ) : __( '● Live mode', 'woo-wallet' ) )
		: __( 'Not configured', 'woo-wallet' );
	const statusColor = masterChecked ? 'var(--ww-accent)' : 'var(--ww-text-muted)';
	...
	return (
		<div style={ {
			...
			>
				...
				<div style={ { flex: 1 } }>
					<p style={ { fontSize: 13, fontWeight: 600, color: 'var(--ww-text)', margin: 0 } }>
						{ group.title }
					</p>
					<p style={ { fontSize: 11, color: statusColor, margin: '2px 0 0' } }>
						{ statusLabel }
					</p>
				</div>
			...
			{ open && (
				<div style={ {
					...
					{ credFields.length > 0 && renderFieldRows( credFields, values, onChange ) }

					{ feeFields.length > 0 && (
						<div style={ {
							borderTop: credFields.length > 0 ? '1px solid var(--ww-border)' : 'none',
							paddingTop: credFields.length > 0 ? 16 : 0, marginTop: 4,
						} }>
							<p style={ {
								fontSize: 11, fontWeight: 600, color: 'var(--ww-text-muted)',
		...
		<div style={ {
			...
			<div style={ { padding: '18px 24px 16px', borderBottom: '1px solid var(--ww-border)' } }>
				<div style={ { display: 'flex', alignItems: 'center', gap: 8 } }>
					...
					<h3 style={ { fontSize: 15, fontWeight: 600, color: 'var(--ww-text-heading)', margin: 0 } }>
						{ __( 'Payout Gateway Credentials', 'woo-wallet' ) }
					</h3>
				</div>
				<p style={ { fontSize: 12, color: 'var(--ww-text-muted)', marginTop: 4 } }>
					{ __( 'Configure API keys for each payout method. Enable the ones you want to offer customers.', 'woo-wallet' ) }
				</p>
			</div>
			<div style={ { padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 10 } }>
				{ groups.map( ( group ) => (
					<GatewayCard key={ group.id } group={ group } values={ values } onChange={ onChange }/>
				) ) }
			</div>
		</div>
	);
...
export default function Panel( { sectionId, schema, values, onChange, onSave, saving, saved, appendChildren = null } ) {
	...
	const handleFieldChange = ( fieldName, val ) => onChange( sectionId, fieldName, val );
		...
		<div style={ { display: 'flex', flexDirection: 'column', gap: 20 } }>
			{ visibleLoose.length > 0 && (
				<div style={ {
					...
					<div style={ {
						...
						{ renderFieldRows( visibleLoose, sectionValues, handleFieldChange ) }
					</div>
				</div>
			) }
			...
			{ fields.length === 0 && (
				<div style={ {
					...
					<p style={ { fontSize: 14, color: 'var(--ww-text-muted)', margin: 0 } }>
