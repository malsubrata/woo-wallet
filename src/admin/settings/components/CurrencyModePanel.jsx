...
export default function CurrencyModePanel( { value, onChange } ) {
	...
	const refresh = useCallback( () => {
		...
		apiFetch( { path: ENDPOINT } )
			...
			.catch( ( err ) => {
				setError(
					err?.message ||
						__( 'Failed to load currency state.', 'woo-wallet' )
				);
				setLoading( false );
			} );
	}, [] );
	...
	if ( loading ) {
		return (
			...
			>
				{ __( 'Loading currency configuration…', 'woo-wallet' ) }
			</div>
		);
	}
	if ( error ) {
	...
	return (
		...
		>
			...
			>
				...
				>
					{ __(
						'Detected provider, currencies, and ledger storage mode.',
						'woo-wallet'
					) }
				</p>
			</div>
			...
			<div style={ { padding: '8px 24px 4px' } }>
				<StateRow
					label={ __( 'Active provider', 'woo-wallet' ) }
					value={ providerLabel }
				/>
				<StateRow
					label={ __( 'Base currency', 'woo-wallet' ) }
					value={ base_currency }
					mono
				/>
				<StateRow
					label={ __( 'Active currency', 'woo-wallet' ) }
			...
			{ showWarning && (
				<div style={ { padding: '0 24px 16px' } }>
					<Banner tone="warning">
						{ __(
							'Per-currency mode is active but no multi-currency plugin is detected. Conversions will fall open (1:1) until a supported plugin is installed.',
							'woo-wallet'
						) }
					</Banner>
				</div>
			...
			{ per_currency_enabled && onChange && (
				...
				>
					<div>
						...
						>
							{ __(
								'Single base normalises every row to the shop currency. Per-currency keeps each row in its source currency and tracks sub-balances independently.',
								'woo-wallet'
							) }
						</p>
					</div>
			...
			{ ! per_currency_enabled && (
				...
				>
					{ __(
						'Per-currency mode is not yet available on this site. Single base mode is in use.',
						'woo-wallet'
					) }
				</div>
			) }
			...
			<ConfirmDialog
				...
				body={
					pendingMode === 'per_currency'
						? __(
								'Existing transaction rows will not be rewritten. Mixed-currency rows from before this change will become real per-currency rows on read. Test on a staging environment before applying to production.',
								'woo-wallet'
						  )
						: sprintf(
								/* translators: %s: base currency code */
								__(
									'New rows will normalise to %s. Existing per-currency rows are preserved but will be excluded from the active-currency balance until a reconcile pass converts them.',
									'woo-wallet'
								),
								base_currency
						  )
