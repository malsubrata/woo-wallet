...
jQuery( function ( $ ) {
	...
	if ( tableEl ) {
		...
		controlsWrap.innerHTML =
			'<input type="date" class="wc-wallet-filter-from" aria-label="' +
			wallet_param.i18n.apply +
			'" max="' +
			new Date().toISOString().slice( 0, 10 ) +
			'">' +
			'<span class="woo-wallet-filter-sep">&ndash;</span>' +
			'<input type="date" class="wc-wallet-filter-to" max="' +
			new Date().toISOString().slice( 0, 10 ) +
			'">' +
			'<button type="button" class="button wc-wallet-filter-clear">&times;</button>';
		tableEl.parentNode.insertBefore( controlsWrap, tableEl );

		const fromInput = controlsWrap.querySelector(
			'.wc-wallet-filter-from'
		);
		const toInput = controlsWrap.querySelector( '.wc-wallet-filter-to' );
		const clearBtn = controlsWrap.querySelector(
			'.wc-wallet-filter-clear'
		);
